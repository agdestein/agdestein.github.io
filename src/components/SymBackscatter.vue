<template>
  <div class="symbs">
    ...
    <div class="symbs-plot" tabindex="0" role="region" aria-label="Scrollable scientific plot">
    ...
    <label class="symbs-probe">Inspect dissipation rate
      <input type="range" min="-0.32" max="0.95" step="0.01" :value="hoverX ?? 0"
        @input="hoverX = Number(($event.target as HTMLInputElement).value)" />
      <output aria-live="polite">{{ hoverX === null ? 'Move to inspect' : series.map(s => `${s.name}: ${fmtDensity(valueAt(s, hoverX!))}`).join(' · ') }}</output>
    </label>
    <p class="symbs-caption">
      ...
      Hover, or use the slider, for exact densities.
    </p>
  </div>
</template>
...
<style>
.symbs-probe { display:grid; gap:8px; margin:12px 0; color:var(--muted); font-size:12px; }
.symbs-probe input { width:100%; accent-color:var(--accent); }
.symbs {
  --symbs-smag: var(--series-4);
  --symbs-clark: var(--series-5);
  --symbs-tbnn: var(--series-2);
