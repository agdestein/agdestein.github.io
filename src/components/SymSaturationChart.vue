<template>
  <div class="symsat">
    ...
    <div class="symsat-plot" tabindex="0" role="region" aria-label="Scrollable scientific plot">
      <svg :viewBox="`0 0 ${W} ${H}`" class="symsat-svg" role="img"
        ...
        <rect v-for="(t, i) in tierX" :key="'hit' + i" :x="xPix(t) - 40" :y="MT" width="80"
          :height="H - MT - MB" fill="transparent" tabindex="0" @click="active = i" @mouseenter="active = i"
    ...
    <p class="symsat-caption">
      {{ mode === 'prior'
        ? 'Relative error of the predicted sub-filter stress against the exact one, on held-out snapshots. The dashed reference line is the training-free conditional-mean estimate of the best possible one-point closure.'
...
<style>
.symsat {
  --symsat-mlp: var(--series-3);
  --symsat-gcnn: var(--series-1);
  --symsat-tbnn: var(--series-2);
  --symsat-smag: var(--series-4);
  --symsat-clark: var(--series-5);
