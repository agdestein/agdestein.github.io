<template>
  <div class="tife">
    ...
    <div class="tife-plot" tabindex="0" role="region" aria-label="Closure error plot; scroll horizontally on small screens">
      <svg :viewBox="`0 0 ${W} ${H}`" class="tife-svg" role="img"
        ...
        <g v-for="s in series" :key="s.key">
          <path :d="linePath(s.vals)" :stroke-dasharray="s.key === 'sp' ? '8 4' : s.key === 'cl' ? '3 4' : s.key === 'nm' ? '10 4 2 4' : undefined" class="tife-line" :class="`tife-l-${s.key}`" />
        ...
        <rect v-for="(c, i) in cfls" :key="'hit' + i" :x="xPix(i) - colHalf" :y="MT"
          ...
          @click="active = i" @mouseenter="active = i" @mouseleave="active = null" @focus="active = i"
          @blur="active = null"
    ...
    <p class="tife-caption">
      ...
      coarse simulation (central coarse flux, log scale). Tap, hover, or tab across the columns
      for exact values.
    </p>
...
<style>
.tife {
  --tife-st: var(--series-1);
  --tife-sp: var(--series-2);
  --tife-cl: var(--series-3);
